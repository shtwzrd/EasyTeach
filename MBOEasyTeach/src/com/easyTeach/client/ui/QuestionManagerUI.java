...
public class QuestionManagerUI {
	...
	private final String[] FILTER = { "Question Type" };
	...
	QuestionManagerPresenter presenter;

	/**
	 * Constructor for building the questionManagerPanel. The panel is built by
	...
	public QuestionManagerUI() {
		this.presenter = new QuestionManagerPresenter();
		buildPanel();
		addActionListeners();
	}

	/**
	 * The getQuestionManagerUI returns the JPanel containing all the components
	 * needed to maintain questions.
	 * 
	 * @return the JPanel, questionManagerPanel. Meant to be used in another
	 *         frame.
	 */
	...
	/**
	 * Builds the center panel consisting of two inner panels. One for an answer
	 * table showing which questions relate to answers and another containing a
	 * panel to filter questions.
	 */
	...
	private void buildAnswerTablePanel() {
		...
		this.answerTablePanel.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED, new Color(0, 0, 0), null),
				"Question's Answers", TitledBorder.CENTER, TitledBorder.TOP,
				new Font("Tahoma", Font.PLAIN, 20)));
		this.lowerCenterPanel.add(this.answerTablePanel, BorderLayout.CENTER);

		this.answerTable.setModel(this.presenter.getAnswerTableModel());
	...
	private void buildFilterPanel() {
		...
		this.filterPanel.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED, new Color(0, 0, 0), null),
				"Filter Questions", TitledBorder.CENTER, TitledBorder.TOP,
				new Font("Tahoma", Font.PLAIN, 20)));
	...
	private void buildUpperCenterPanel() {
		...
		upperCenterPanel.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED, new Color(0, 0, 0), null), "Questions",
	...
	private void addActionListeners() {
		...
		this.questionTable.addMouseListener(listener);
	}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == QuestionManagerUI.this.btnAddQuestion) {
				...
				JOptionPane.showMessageDialog(null,
						HelpPresenter.getQuestionManagerHelp(),
						HelpPresenter.getQuestionManagerTitle(),
						JOptionPane.PLAIN_MESSAGE, HelpPresenter.getHelpIcon());
			}

			else if (e.getSource() == QuestionManagerUI.this.btnFilter) {
				String column = QuestionManagerUI.this.filterBox
						.getSelectedItem().toString();
				String by = QuestionManagerUI.this.txtFilter.getText();

				QuestionManagerUI.this.presenter.filter(column, by);
			}
		}

		@Override
		public void mouseClicked(MouseEvent arg0) {
			if (arg0.getSource() == QuestionManagerUI.this.questionTable) {
				QuestionManagerUI.this.presenter
						.setSelectedQuestion(QuestionManagerUI.this.questionTable
